...
# template erzeuchen: foto machem, anzahl zeilen einstellen (achtung ungleiche skalierung, OCR?), welche Spalte ist REF (automatisch ermittlen aus template)
    ROIs einzeichnen.
                ...
                if M is not None:
                    ...
                    if inliers > MIN_INLIER_COUNT:
                        ...
                        if area > MIN_AREA:
                            ...
                            even_rois = medians[1::2]
                            odd_rois = medians[0::2]
                            ...
                            for i, (r, lab) in enumerate(zip(rects, medians)):
                                ...
                                lab_bgr = cv2.cvtColor(np.uint8([[[lab[0] * 255 / 100, lab[1] + 128, lab[2] + 128]]]),
                                                       cv2.COLOR_Lab2BGR)[0, 0]
                                cv2.rectangle(annotated, (x2 + 2, y1), (x2 + 2 + 20, y1 + 20),
                                              tuple(int(v) for v in lab_bgr), -1)
                                cv2.putText(annotated, str(i + 1), (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                            (255, 255, 255), 1)
